  ...
  const renderSectionContent = () => {
    switch (activeSection) {
          ...
          <div className="space-y-6">
            <Card className="border border-border/60 bg-card/80 animate-fade-up" style={{ animationDelay: "0.05s" }}>
              ...
              <CardContent className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-lg border border-border/40 bg-background/40 p-4 text-sm text-muted-foreground">
                  ...
                  <p className="mt-2">
                    Combine os rituais diarios com lembretes personalizados. Defina alarmes leves para nunca esquecer o momento do casal.
                  </p>
                </div>
                <div className="rounded-lg border border-border/40 bg-background/40 p-4 text-sm text-muted-foreground">
                  <p className="font-semibold text-foreground">Planos curtos</p>
                  <p className="mt-2">
                    Receba em breve planos de 7, 14 e 30 dias para avanco rapido, ajustados ao tempo que voces tem disponivel.
                  </p>
                </div>
              </CardContent>
            </Card>

            <Card className="border border-border/60 bg-card/80 animate-fade-up" style={{ animationDelay: "0.1s" }}>
              <CardHeader>
                <CardTitle>Novidades recentes</CardTitle>
                <CardDescription>Atualizacoes adicionadas pelo time editorial aparecem aqui em tempo real.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {announcementItems.length ? (
                  announcementItems.map((item, index) => (
                    <div
                      key={item.id}
                      className="rounded-lg border border-border/40 bg-background/40 p-4 transition hover:border-primary/50 hover:bg-primary/5"
                  ...
                  <p className="text-sm text-muted-foreground">
                    Sem atualizacoes recentes. Confira novamente mais tarde.
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {overviewHighlights.map((item, index) => (
                <Card
                  key={item.title}
                  className="border border-border/60 bg-card/80 animate-fade-up"
      ...
      default:
        return (
          <div className="space-y-6">
            <Card className="border border-border/60 bg-card/80 animate-fade-up">
              ...
              <CardContent className="space-y-6">
                {isLoading ? (
                  ...
                  <Tabs defaultValue={firstCategory} className="space-y-6">
                    <TabsList className="flex w-full gap-2 overflow-x-auto rounded-full border border-border/40 bg-background/60 p-1">
                      {categories.map((category) => (
                        <TabsTrigger
                          key={category}
                          value={category}
                          className="capitalize rounded-full px-4 py-2 text-sm"
                        >
                          {category}
                        </TabsTrigger>
                      ))}
                    ...
                    {categories.map((category, index) => (
                      <TabsContent key={category} value={category} className="space-y-4">
                        <div
                          className="rounded-xl border border-border/40 bg-background/40 p-4 text-sm text-muted-foreground animate-fade-up"
                          style={{ animationDelay: `${0.05 + index * 0.05}s` }}
                        >
                          <p>
                            Explore {category} com experiencias pensadas para diferentes momentos e niveis de energia. Escolha uma receita abaixo e siga o passo a passo.
                          </p>
                        </div>
                        <div className="space-y-4">
                          {recipesByCategory[category].map((recipe, recipeIndex) => (
                            <article
                              key={recipe.id}
                              className="flex flex-col gap-4 rounded-xl border border-border/60 bg-background/60 p-4 transition hover:border-primary/50 hover:shadow-lg md:flex-row md:items-center"
                              style={{ animationDelay: `${0.1 + recipeIndex * 0.04}s` }}
                            >
                              {recipe.image_url && (
                                <img
                                  src={recipe.image_url}
                                  alt={`Imagem da receita ${recipe.title}`}
                                  className="h-28 w-full rounded-lg object-cover md:h-32 md:w-40"
                                />
                              )}
            ...
            <div className="grid gap-4 md:grid-cols-2 animate-fade-up" style={{ animationDelay: "0.15s" }}>
